...
这个公开仓库把两个真实 ESP32-C3 项目整理成两条可解释链路。
...
公开版本保留解析层和扫描线索。原项目中的真实 MAC 地址、业务设备名称、控制对象和第三方连接头文件不放入公开仓库。

## 为什么不是直接开源原工程

直接开源原工程会带出真实设备名、调试参数、MAC 地址、协议细节和可能不适合公开的交付信息。这个仓库采用 clean-room 方式重写公开版本，只保留通用流程和可验证代码。
